...
print("#" * 40)
# calcute day age
getYear = int(input("Wahts Is birth year ? "))
getmonth = int(input("Wahts Is birth Month ? "))
getDay = int(input("Wahts Is birth Day ? "))


def calcuteDate(year, month, day):
    myBirthDay = datetime.datetime(year, month, day)
    dateNow = datetime.datetime.now()
    mydays = (dateNow - myBirthDay).days
    return mydays


print(calcuteDate(getYear, getmonth, getDay))
